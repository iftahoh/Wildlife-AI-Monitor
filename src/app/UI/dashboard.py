...
# --- 1. שלב קריטי: תיקון הנתיבים (חייב להיות לפני ה-imports מ-src) ---
# הפקודה הזו אומרת לפייתון: "תחפש קבצים גם בתיקייה הראשית של הפרויקט (3 רמות למעלה)"
current_dir = os.path.dirname(os.path.abspath(__file__))
root_dir = os.path.abspath(os.path.join(current_dir, '../../..'))
sys.path.append(root_dir)

# --- 2. עכשיו אפשר לייבא בבטחה את הקבצים שלך ---
try:
    from src.app.DB.database import get_all_sightings, run_custom_query
    from src.app.AI.sql_agent import text_to_sql
except ImportError as e:
    st.error(f"שגיאת ייבוא (Import Error): {e}")
    st.stop()
...
with tab3:
    ...
    try:
        ...
        if rows:
            ...
            # חישוב: סכימת הכמויות לפי סוג חיה
            ...
            with col_b:
                ...
                bar = alt.Chart(hourly).mark_bar().encode(
                    ...
                    y=alt.Y("count", title="מספר אירועים"),
        ...
        st.error(f"שגיאה בחישוב סטטיסטיקה: {e}")
...
with tab4:
    ...
    st.markdown("שאל שאלות חופשיות על המידע שנאסף (למשל: 'כמה צבאים ראינו השבוע?').")

    question = st.text_input("מה תרצה לדעת?", placeholder="הקלד שאלה...")

    if st.button("שאל את ה-AI ✨") and question:
        with st.spinner("ה-AI כותב שאילתה..."):
            try:
                # 1. שליחה ל-AI לקבלת SQL
                generated_sql = text_to_sql(question)
                st.code(generated_sql, language="sql")

                # 2. הרצת השאילתה מול הדאטה-בייס
                data, columns, error = run_custom_query(generated_sql)

                if error:
                    st.error(f"שגיאה בהרצה: {error}")
                elif data:
                    df_res = pd.DataFrame(data, columns=columns)
                    st.dataframe(df_res, use_container_width=True)
                    st.success(f"נמצאו {len(data)} תוצאות")
                else:
                    st.warning("השאילתה רצה בהצלחה אך לא נמצאו תוצאות מתאימות.")
